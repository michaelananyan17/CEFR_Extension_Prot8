...
async function rewritePageContent(apiKey, targetLevel) {
    try {
        ...
        // Process each text element in parallel batches
    ...
    for (let i = 0; i < elementsArray.length; i += batchSize) {
        ...
        batchResults.forEach(result => {
            if (result.status === 'fulfilled' && result.value.success) {
                ...
                // Steps 4-6: Replace text content
...
// Store original text content - SMARTER element selection
function storeOriginalTexts() {
    ...
    let index = 0;
    let processedFirstParagraph = false;
    
    textElements.forEach((element) => {
        // NEW: Smarter element processing
        if (shouldProcessElementWithChildren(element) &&
            element.textContent && 
            ...
            // NEW: Don't skip first paragraph - process it!
            // We'll process ALL qualifying paragraphs, including the first one
            
            // Step 5: Store all formatting information
            ...
            // Track that we've processed at least one paragraph
            if (!processedFirstParagraph && element.tagName.toLowerCase() === 'p') {
                processedFirstParagraph = true;
            }
        }
    });
...
// NEW: Smarter element processing that handles elements with children
function shouldProcessElementWithChildren(element) {
    const tagName = element.tagName.toLowerCase();
    
    // Always process headings - they're usually simple
    if (tagName.match(/^h[1-6]$/)) {
        return true;
    }
    
    // For paragraphs and divs, check if they're mainly text content
    if (tagName === 'p' || tagName === 'div') {
        const childElements = element.children;
        
        // If no children, definitely process
        if (childElements.length === 0) {
            return true;
        }
        
        // If has children, check if they're mainly inline elements or images
        let hasBlockElements = false;
        let hasTooManyChildren = false;
        
        for (let child of childElements) {
            const childTag = child.tagName.toLowerCase();
            
            // Block elements that break text flow
            if (['div', 'p', 'ul', 'ol', 'table', 'blockquote'].includes(childTag)) {
                hasBlockElements = true;
                break;
            }
            
            // Too many children might indicate complex structure
        ...
        // Process if no block elements and not too many children
        return !hasBlockElements && !hasTooManyChildren;
    }
    
    return true;
}

// Replace element text content - handles elements with simple children
function replaceElementTextContent(element, newText) {
    ...
    try {
        // Step 4: Delete all original text + Step 6: Apply rewritten text
        
        // If element has no children or only simple inline elements, use textContent
        if (childElements.length === 0 || hasOnlyInlineElements(element)) {
            element.textContent = newText;
        } else {
            // For elements with children, be more careful
            // Remove text nodes but keep child elements
            const textNodes = getTextNodes(element);
            textNodes.forEach(node => node.parentNode.removeChild(node));
            
            // Add the rewritten text at the beginning
            const textNode = document.createTextNode(newText + ' ');
            element.insertBefore(textNode, element.firstChild);
        }
        
        // Step 7: Verify no original text remains
        setTimeout(() => {
            const currentText = element.textContent.trim();
            let hasOriginalText = false;
            
            originalTexts.forEach(item => {
                if (item.element === element && currentText.includes(item.originalText)) {
                    hasOriginalText = true;
                }
            });
            
            if (!hasOriginalText) {
                // Step 8: Present to user with smooth transition
                ...
                // Fallback: restore original and use simple replacement
        ...
        console.error('Error replacing text content:', error);
        // Fallback: simple text replacement
        element.textContent = newText;
    }
}

// NEW: Check if element only contains inline elements
function hasOnlyInlineElements(element) {
    const inlineTags = [
        'span', 'strong', 'em', 'b', 'i', 'u', 'a', 'code', 'mark', 
        'small', 'sub', 'sup', 'time', 'abbr', 'cite', 'dfn'
    ];
...
// Check if element should be processed (basic filters)
function shouldProcessElement(element) {
    ...
    const excludeTags = ['script', 'style', 'noscript', 'iframe', 'code', 'pre'];
    if (excludeTags.includes(tagName)) {
        return false;
    }
    
    // Don't process navigation and UI elements
    const excludeClassPatterns = [
        'nav', 'menu', 'header', 'footer', 'sidebar', 'aside', 
        'button', 'btn', 'form', 'input', 'select', 'label'
    ];
    
    for (const pattern of excludeClassPatterns) {
...
// Enhanced text rewriting with smart preservation
...
- PRESERVE these exactly as they appear in the original:
  * Proper names (people, companies, brands)
  * Dates (March 27th 1996, Wednesday, etc.)
  * Times (9:33am, etc.)
  * Locations (San Fernando Valley, Ventura Boulevard, etc.)
  * Numbers and measurements (25,000lb, etc.)
  * Technical terms (Brinks armoured car, etc.)
  * Addresses and specific references
...
Rewritten text (STRONGLY representing ${targetLevel} level, preserving specific terms):`;
        ...
        const response = await fetch('https://api.openai.com/v1/chat/completions', {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    {
                        role: 'system',
                        content: `You are a professional text rewriter that STRONGLY adapts content to specific CEFR English levels. You must EXAGGERATE the level characteristics to make the ${targetLevel} level very clear and distinct. You MUST preserve all proper names, dates, times, locations, numbers, and technical terms exactly as they appear in the original text without any changes.`
                    },
                    {
                        role: 'user',
                        content: prompt
